      ...
      try {
        ...
        activeStrategyPipelines.push({
          ...
          config: {
            ...
            skipV3Filter: strategy.key === 'GOD_SMC_V1' || strategy.key === 'FVG_SCALP_V1' || strategy.key === 'FVG_SCALP_AGG_V1' || strategy.key === 'MOMENTUM_SCALP_V1',
...
async function __processIStrategyDecision_removed(..._: any[]) { const strategy: any = null, symbol: any = null, profileKey: any = null; try {
    ...
    if (!result.orders || result.orders.length === 0) {
      ...
      // Always log to decisions table (every tick) so engine monitor shows GOD strategy
      ...
      // Journal the skip (deduplicated — only when reason changes)
