/**
 * FVG Scalp AGGRESSIVE Strategy - IStrategy Implementation
 *
 * High-frequency version: 5-15 trades per day. Looser FVG detection
 * (no displacement requirement), wider sessions, lower M1 confirmation bar.
 * Same risk management as conservative (3 consec loss, $200 daily cap).
 *
 * Differences from Conservative:
 *   - No LuxAlgo displacement check on FVG candle (accepts ALL valid gaps)
 *   - M1 confirmation: body >30% (was >40%)
 *   - Near-24h sessions (only skip 21-01 UTC dead zone)
 *   - Proximity check 5x FVG size (was 3x)
 *
 * Logic:
 *   1. Detect H4 bias (swing structure)
 *   2. Find M5 FVGs in bias direction (any gap, not just displacement)
 *   3. Enter when M1 price bounces off FVG as support/resistance
...
const logger = new Logger('FVGScalpAgg');
...
export class FVGScalpAggressiveStrategy implements IStrategy {
  readonly key = 'FVG_SCALP_AGG_V1';
  readonly displayName = 'FVG Scalp Aggressive';
  ...
  constructor(profile: StrategyProfile) {
    ...
    logger.info(`[FVGScalpAgg] Initialized: R:R=${this.riskRewardTarget}, minFVG=${this.minFVGSizeMultiplier}x, SL=${this.minSLPoints}-${this.maxSLPoints}pts`);
  }
  ...
  async execute(context: StrategyContext): Promise<StrategyResult> {
    ...
    logger.info(
      `[FVGScalpAgg] ${symbol} ${session.label}: ${entry.direction.toUpperCase()} @ ${entry.entryPrice.toFixed(2)} ` +
